...
//新增
const roleRef = ref<InstanceType<typeof roleDrawer> | null>()
const handleCreate = () => {
  roleRef.value!.openDrawer()
}
...
const handleChange = () => {
  ...
  roleRef.value!.changeOpen(id)
}

const handleDelete = () => {
  if (selectRow.value.length !== 1) {
    messageBox.confirm({
      message: '请选择一条需要删除的角色信息数据',
      title: '提示',
      options: {
        showCancelButton: false,
        showConfirmButton: false,
