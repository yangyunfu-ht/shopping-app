...
//新增
const brandRef = ref<InstanceType<typeof brandDrawer> | null>()
const handleCreate = () => {
  brandRef.value!.openDrawer()
}
...
const handleChange = () => {
  ...
  brandRef.value!.changeOpen(id)
}

const handleDelete = () => {
  if (selectRow.value.length !== 1) {
    messageBox.confirm({
      message: '请选择一条需要删除的品牌信息数据',
      title: '提示',
      options: {
        showCancelButton: false,
        showConfirmButton: false,
